...
<?php
    ...
    $paginationParams = 'MilestoneStatuses';
?>
<div class="box">
    <div class="box-header">
        <h3 class="box-title"><?= __('MilestoneStatuses') ?></h3>
        <form class="supp-form textgrey margin-top10" role="form" action="#">
        	<div class="btn-group pull-right padding-right15">
            	...
            	<?= $this->Html->link(__('<i class="fa fa-fw fa-plus"></i>'), ['action' => 'add'], ['class' => 'btn btn-success','escape'=>false]) ?>
			</div>
    ...
    <?php     
    $class[$this->request->params['paging']['MilestoneStatuses']['sort']] = $this->request->params['paging']['MilestoneStatuses']['direction'];
    $status_class = 'sorting';
    switch(key($class)) {
		case 'MilestoneStatuses.status' : 
			$status_class= 'sorting_' . $class['MilestoneStatuses.status'];
			break;
	}
    ?>
    <div class="box-body">
        <table id="example1" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
            ...
            <tbody>
                <?php 
                	$pages = $this->Paginator->request->params['paging']['MilestoneStatuses'];
                	$cnt = (($pages['page'] - 1) * $pages['perPage']) + 1;
                	foreach ($milestone_statuses as $milestone_status): 
                ?>
                <tr>
                    <td>
                        <?= $cnt ?>
                    </td>
                    <td><?= h($milestone_status->status) ?></td>
                    <td class="actions">
                        <?php 
                            if($can_edit) {
                                echo $this->Html->link(null, array_merge(['action' => 'edit', $milestone_status->id], $paginateParams), ['class' => 'fa fa-pencil']);
                            } else {
                        ...
                        <?php
                        	if($can_delete) {
                        		echo $this->Html->link(null, array_merge(['action' => 'delete', $milestone_status->id], $paginateParams), ['class' => 'fa fa-times', 'confirm' => __('Are you sure you want to delete the record')]);
                        	}  else {
        ...
        <?= $this->element('Admin' . DS . 'pagination', ['module' => 'MilestoneStatuses', 'paginateParams' => $paginateParams]); ?>
    </div>
</div>
<script>
  $(function () {
