...
<div class="box">
    <div class="box-header">
        <h3 class="box-title"><?= __('Designations') ?></h3>
        <form class="supp-form textgrey margin-top10" role="form" action="#">
	        ...
	        <div class="form-group padding-top50">
	          <label class="col-sm-1 control-label search-fields-pad" for="inputLastName3" style="width:130px;">Designation Name</label>
	          <div class="col-sm-2">
	            <?php 
    ...
    <?php     
    $class[$this->request->params['paging']['Designations']['sort']] = $this->request->params['paging']['Designations']['direction'];
    $designation_name_class = 'sorting';
    switch(key($class)) {
		case 'Designations.designation_name' : 
			$designation_name_class= 'sorting_' . $class['Designations.designation_name'];
			break;
	}
    ?>
    <div class="box-body">
        <table id="example1" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
            <thead>
                <tr>
                    ...
                    <th class="<?=$designation_name_class ?>"><?= $this->Paginator->sort('designation_name', __('Designation Name')); ?></th>
                    <th class="actions no-sort"><?= __('Actions') ?></th>
                </tr>
            </thead>
            <tbody>
                <?php 
                	$pages = $this->Paginator->request->params['paging']['Designations'];
                ...
                <tr>
                    ...
                    <td><?= h($designation->designation_name) ?></td>
                    <td class="actions">
                        <?php 
                            if($can_edit) {
                                echo $this->Html->link(null, array_merge(['action' => 'edit', $designation->id], $paginateParams), ['class' => 'fa fa-pencil']);
                            } else {
                        ...
                        <?php
                        	if($can_delete) {
                        		echo $this->Html->link(null, array_merge(['action' => 'delete', $designation->id], $paginateParams), ['class' => 'fa fa-times', 'confirm' => __('Are you sure you want to delete the record')]);
                        	}  else {
        ...
        <?= $this->element('Admin' . DS . 'pagination', ['module' => 'Designations', 'paginateParams' => $paginateParams]); ?>
    </div>
</div>
<script>
  $(function () {
